...
//Yeah, i did not succeed in trying to make it in multiple files...
void main() {
...
final pastSessionsProvider =
    StateNotifierProvider<Session, Str>((ref) => Session());
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return ListTile(
                    ...
                    subtitle: Text(pastSessions.str[index],
...
class _SessionPreferencesScreenState extends State<SessionPreferencesScreen> {
  ...
  int hotInterval = 20;
  int coldInterval = 20;
  int intervalPrecisionIndex = 0;
  int intervalPrecision = 5;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title:
            const Text("Session Preferences", style: TextStyle(fontSize: 48.0)),
      ),
      body: Padding(
        ...
        child: Column(
          children: <Widget>[
                ...
                Expanded(
                  child: Slider(
                    ...
                    onChanged: (value) {
                      setState(() {
                        coldInterval =
                            min(coldInterval, ((value.round() * 30)).round());
                        hotInterval =
                            min(hotInterval, ((value.round() * 30)).round());
            ...
            Row(
              children: <Widget>[
                const Text("Hot Interval (seconds):",
                ...
                Expanded(
                  child: Slider(
                    ...
                    min: 0.0,
                    max: duration * 30,
                    divisions: ((duration * 30) / intervalPrecision).round(),
            ...
            Row(
              children: <Widget>[
                const Text("Cold Interval (seconds):",
            ...
            Row(
              children: <Widget>[
                const Text("Interval precision (seconds):",
                    style: TextStyle(fontSize: 24.0, color: Colors.green)),
                Expanded(
                  child: Slider(
                    value: intervalPrecisionIndex.toDouble(),
                    min: 0,
                    max: 3,
                    divisions: 3,
                    label: intervalPrecision.toString(),
                    onChanged: (value) {
                      setState(() {
                        intervalPrecision =
                            [5, 10, 50, 150].elementAt(value.round()).round();
                        intervalPrecisionIndex = value.round();
                      });
                    },
                  ),
                ),
...
class SessionOverviewScreen extends StatefulWidget {
  ...
  const SessionOverviewScreen(
      ...
      required this.hotInterval,
      required this.coldInterval,
      super.key});
  @override
...
class SessionOverviewScreenState extends State<SessionOverviewScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Session overview", style: TextStyle(fontSize: 48.0)),
      ),
      body: Padding(
        ...
        child: Column(
          children: <Widget>[
            Text(
              "Session duration: ${widget.getDuration()} minutes.",
              style: const TextStyle(fontSize: 24.0),
            ),
            const SizedBox(
              height: 20.0,
            ),
            Text("Hot interval: ${widget.getHotInterval()} seconds.",
                style: const TextStyle(fontSize: 24.0, color: Colors.red)),
            const SizedBox(
              height: 20.0,
            ),
            Text("Cold interval: ${widget.getColdInterval()} seconds.",
...
class ActiveSessionScreenState extends State<ActiveSessionScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Active Session", style: TextStyle(fontSize: 48.0)),
      ),
      body: Center(
        child: Column(
          children: <Widget>[
...
class SessionSummaryScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
          title:
              const Text("Session Summary", style: TextStyle(fontSize: 48.0))),
      body: Center(
        child: Column(
          children: <Widget>[
            ...
            ElevatedButton(
                onPressed: () {
                  rate =
                      "Duration: $duration minutes, Hot interval: $hotInterval seconds, Cold interval: $coldInterval seconds";
