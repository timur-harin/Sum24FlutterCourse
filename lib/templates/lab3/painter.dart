...
class WaterProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // TODO - Using size and waterIntakeLevel to calculate the water level DONE
    final waterLevel = (waterIntakeLevel/3000) * (size.height+149);

    final paint = Paint()
      ..color = Colors.blueAccent.withOpacity(0.5)
      ..style = PaintingStyle.fill;

    // TODO - Draw the water level on the canvas using rectangle and size from waterLevel DONE

    canvas.drawRect(
      Rect.fromLTRB(0, size.height - waterLevel+64, size.width, size.height+64),
      paint,
    );
  }
